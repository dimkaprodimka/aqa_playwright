...
test.describe('Tests for QAuto user registration', { tag: ["@smoke", "@regression"] }, () => {
  test('User can successfully register with correct creds', async ({ page }) => {
    ...
    await register_page.userRegistration(users.user1.name, users.user1.lastname, users.user1.email, 
      users.user1.password, users.user1.repeat_password)
    await expect(register_page.elements.registerMessage).toHaveText('Registration complete')
  })

  test.skip('User can not register, using special characters in name and lastname', async ({ page }) => {
    const register_page = new RegistrationPage(page)
  ...
  test.skip('User can not register, using invalid email format', async ({ page }) => {
    const register_page = new RegistrationPage(page)
  ...
  test.skip('User can not register with weak password', async ({ page }) => {
    const register_page = new RegistrationPage(page)
...
one integer, one capital, and one small letter')
  ...
  test.skip('User can not register with non-matching passwords', async ({ page }) => {
    const register_page = new RegistrationPage(page)
  ...
  test.skip('User can not register with empty creds', async ({ page }) => {
    const register_page = new RegistrationPage(page)
